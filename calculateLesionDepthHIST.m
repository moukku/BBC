function [lesiondepthratio,lesiondepthmm,rowcol_lesion,BWcart,sub_cartsurf_smoothed,sub_lesion]=calculateLesionDepthHIST(BWIIrot,sub_realcartsurfHIST,sub_CartilageSurfaceHIST,sub_CartilageBoneInterfaceHIST,pixelspermm)
...
%    =calculateLesionDepthHIST(IIBW,sub_realcartsurfHIST,sub_CartilageSurfaceHIST,sub_CartilageBoneInterfaceHIST,pixelspermm);
  ...
  sub_cartsurf_smoothed=sub_CartilageSurfaceHIST;
  sub_cartbone=sub_CartilageBoneInterfaceHIST;
  ...
  %If cartilage is flat and initial surface has been set a bit
  %too low this prevents an error in calculation
  if sum(abs(Lesion_depthratio))==0
    Lesion_depthratio(round(end/2))=0.0001;
    Lesion_depthratio(round(end/2))=1/pixelspermm;
  end
